...
describe("skills", () => {
	describe("loadSkillsFromDir", () => {
		...
		it("should warn when name contains invalid characters", () => {
			...
			expect(skills).toHaveLength(1);
			expect(diagnostics.some((d: ResourceDiagnostic) => d.message.includes("无效字符"))).toBe(true);
		});
		...
		it("should warn when name exceeds 64 characters", () => {
			...
			expect(skills).toHaveLength(1);
			expect(diagnostics.some((d: ResourceDiagnostic) => d.message.includes("超过 64 个字符"))).toBe(true);
		});
		...
		it("should warn and skip skill when description is missing", () => {
			...
			expect(diagnostics.some((d: ResourceDiagnostic) => d.message.includes("description 为必填项"))).toBe(true);
		});
		...
		it("should warn when name contains consecutive hyphens", () => {
			...
			expect(skills).toHaveLength(1);
			expect(diagnostics.some((d: ResourceDiagnostic) => d.message.includes("连续连字符"))).toBe(true);
		});
	...
	describe("loadSkills with options", () => {
		...
		it("should warn when skill path does not exist", () => {
			...
			expect(skills).toHaveLength(0);
			expect(diagnostics.some((d: ResourceDiagnostic) => d.message.includes("不存在"))).toBe(true);
		});
