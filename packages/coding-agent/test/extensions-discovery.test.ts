...
describe("extensions discovery", () => {
	...
	it("loads .js extensions that import typebox and register tools", async () => {
		const fixturePath = path.join(__dirname, "fixtures", "js-typebox-extension.js");

		const result = await discoverAndLoadExtensions([fixturePath], tempDir, tempDir);

		expect(result.errors).toHaveLength(0);
		expect(result.extensions).toHaveLength(1);
		expect(path.basename(result.extensions[0].path)).toBe("js-typebox-extension.js");
		expect(result.extensions[0].tools.has("js-tool")).toBe(true);
	});
