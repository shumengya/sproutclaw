...
import { handlePackageCommand, PACKAGE_COMMANDS_DISABLED } from "../src/package-manager-cli.ts";

const describePackageCommands = PACKAGE_COMMANDS_DISABLED ? describe.skip : describe;

describe("package commands (disabled)", () => {
	it("returns not implemented for install", async () => {
		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		process.exitCode = undefined;
		try {
			expect(await handlePackageCommand(["install", "npm:@foo/bar"])).toBe(true);
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("该命令暂未实现");
	...
	it("returns not implemented for update variants", async () => {
		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		process.exitCode = undefined;
		try {
			for (const args of [["update"], ["update", "--self"], ["update", "--all"], ["list"]]) {
				process.exitCode = undefined;
	...
	it("still shows subcommand help", async () => {
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
		process.exitCode = undefined;
		try {
			expect(await handlePackageCommand(["install", "--help"])).toBe(true);
...
describePackageCommands("package commands", () => {
	let tempDir: string;
	let agentDir: string;
