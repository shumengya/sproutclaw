...
describe("Coding Agent Tools", () => {
	...
	describe("read tool", () => {
		...
		it("should truncate files exceeding line limit", async () => {
			...
			expect(output).toContain("[显示第 1-2000 / 2500 行。使用 offset=2001 继续。]");
		});
		...
		it("should truncate when byte limit exceeded", async () => {
			...
			expect(output).toMatch(new RegExp(String.raw`\[显示第 1-\d+ / 500 行（.* 限制）。使用 offset=\d+ 继续。\]`));
		});
		...
		it("should handle limit parameter", async () => {
			...
			expect(output).toContain("[文件中还有 90 行。使用 offset=11 继续。]");
		});
		...
		it("should handle offset + limit together", async () => {
			...
			expect(output).toContain("[文件中还有 40 行。使用 offset=61 继续。]");
		});
		...
		it("should detect image MIME type from file magic (not extension)", async () => {
			...
			expect(getTextOutput(result)).toContain("已读取图片文件 [image/png]");
	...
	describe("write tool", () => {
		it("should write file contents", async () => {
			...
			expect(getTextOutput(result)).toContain("已成功写入");
	...
	describe("edit tool", () => {
		it("should replace text in file", async () => {
			...
			expect(getTextOutput(result)).toContain("已成功替换");
			expect(result.details).toBeDefined();
		...
		it("should include ENOENT when the edit target does not exist", async () => {
			...
			).rejects.toThrow(`无法编辑文件：${missingFile}。错误码：ENOENT。`);
		});
		...
		it("should fail when edits is empty", async () => {
			...
			).rejects.toThrow(/edits 必须至少包含一处替换/);
		});
		...
		it("should include EACCES for read-only files", async () => {
			...
			).rejects.toThrow(new RegExp(`无法编辑文件：${testFile.replace(/\\/g, "\\\\")}。错误码：(EACCES|EPERM)。`));
		});
		...
		it("should include the original error message for unknown edit access errors", async () => {
			...
			).rejects.toThrow("无法编辑文件：broken.txt。Error: disk offline。");
		});
		...
		it("should include ENOENT in diff preview for missing files", async () => {
			...
			expect(result).toEqual({ error: `无法编辑文件：${missingFile}。错误码：ENOENT。` });
		});
		...
		it("should include EACCES in diff preview for unreadable files", async () => {
			...
			if ("error" in result) {
				expect(result.error).toMatch(
					new RegExp(`无法编辑文件：${unreadableFile.replace(/\\/g, "\\\\")}。错误码：(EACCES|EPERM)。`),
				);
			} else {
				// Windows chmod often cannot make a file unreadable for the current user.
	...
	describe("bash tool", () => {
		...
		it("should handle command errors", async () => {
			await expect(bashTool.execute("test-call-9", { command: "exit 1" })).rejects.toThrow(
				/(命令以退出码 1 结束|Command failed|code 1)/,
			);
		});
		...
		it("should include full output path for truncated timeout and abort errors", async () => {
			for (const testCase of [
				{ error: "timeout:5", expected: "命令在 5 秒后超时" },
				{ error: "aborted", expected: "命令已中止" },
			]) {
				...
				expect(message).toMatch(new RegExp(String.raw`\[显示第 \d+-\d+ / \d+ 行。完整输出：`));
				expect(message).not.toContain("完整输出：undefined");
				const fullOutputPath = message.match(/完整输出：([^\]\n]+)/)?.[1];
		...
		it("should not count a trailing newline as an extra truncated bash output line", async () => {
			...
			expect(output).toMatch(new RegExp(String.raw`\[显示第 2001-4000 / 4000 行。完整输出：`));
		...
		it("should persist full output when truncation happens by line count only", async () => {
			...
			expect(result.details?.truncation?.truncatedBy).toBe("lines");
			expect(fullOutputPath).toBeDefined();
			expect(output).toMatch(new RegExp(String.raw`\[显示第 \d+-\d+ / \d+ 行。完整输出：`));
			expect(output).not.toContain("完整输出：undefined");
	...
	describe("grep tool", () => {
		...
		it("should respect global limit and include context lines", async () => {
			...
			expect(output).toContain("[已达 1 匹配上限。使用 limit=2 获取更多，或缩小模式范围]");
		...
		it("should treat flag-like patterns as search text", async () => {
			...
			expect(getTextOutput(result)).toContain("未找到匹配");
	...
	describe("find tool", () => {
		...
		it("should treat flag-like patterns as search text", async () => {
			...
			expect(getTextOutput(result)).toContain("未找到匹配该模式的文件");
		});
	});
