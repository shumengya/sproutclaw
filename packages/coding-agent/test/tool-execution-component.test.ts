...
describe("ToolExecutionComponent parity", () => {
	...
	test("collapsed built-in edit keeps title (not a blank status bar)", () => {
		const component = new ToolExecutionComponent(
			"edit",
			"tool-edit-collapsed",
		...
		// Diff body stays hidden while collapsed
	...
	test("bash renderer does not duplicate final full output truncation details", async () => {
		...
		expect(rendered).not.toContain("[显示第 2001-4000 / 4000 行。完整输出：");
	});
