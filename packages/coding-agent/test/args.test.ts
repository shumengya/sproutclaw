...
describe("parseArgs", () => {
	...
	describe("--config flag", () => {
		test("parses --config with value", () => {
			const result = parseArgs(["--config", "SearchLens", "-p", "hi"]);
			expect(result.config).toBe("SearchLens");
			expect(result.print).toBe(true);
			expect(result.messages).toEqual(["hi"]);
		});

		test("parses --config=name syntax", () => {
			const result = parseArgs(["--config=searchlens", "hello"]);
			expect(result.config).toBe("searchlens");
			expect(result.messages).toEqual(["hello"]);
		});

		test("reports error when --config has no value", () => {
			const result = parseArgs(["--config"]);
			expect(result.config).toBeUndefined();
			expect(result.diagnostics).toContainEqual({ type: "error", message: "--config requires a value" });
		});
	});
