...
describe("extension factory cache", () => {
	...
	it("re-imports uncached .js extensions so reload bypasses the ESM module cache", async () => {
		const { root, cwd } = fixture("direct-js");
		writeFileSync(join(root, "package.json"), `${JSON.stringify({ type: "module" })}\n`);
		const extensionPath = join(root, "counting.js");
		writeCountingExtension(extensionPath);
