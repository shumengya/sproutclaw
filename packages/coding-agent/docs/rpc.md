...
- `setWorkingMessage()`, `setWorkingIndicator()`, `setFooter()`, `setHeader()`, `setEditorComponent()`, `setToolsExpanded()`, `setHideThinkingBlock()` are no-ops
- `getEditorText()` returns `""`
- `getToolsExpanded()` returns `false`
- `getHideThinkingBlock()` returns `false`
