...
sproutai does not check pi.dev for updates unless `PI_CHECK_VERSION=1`. Set `PI_SKIP_VERSION_CHECK=1` to skip that request even when opted in. Use `--offline` or `PI_OFFLINE=1` to disable all startup network operations described here, including package update checks.
