...
JavaScript entries (`.js` / `.mjs`) are loaded with native `import()`. TypeScript still uses [jiti](https://github.com/unjs/jiti), so it works without compilation.
...
// Thinking block visibility (persists to settings)
const wasHidden = ctx.ui.getHideThinkingBlock();
ctx.ui.setHideThinkingBlock(true);   // Collapse thinking to the hidden label
ctx.ui.setHideThinkingBlock(false);  // Show full thinking content
ctx.ui.setHideThinkingBlock(wasHidden);
