...
sproutai does **not** check pi.dev for updates by default. Set `PI_CHECK_VERSION=1` to opt in to `https://pi.dev/api/latest-version`. `PI_SKIP_VERSION_CHECK=1` still skips that request even when opted in.

Use `--offline` or `PI_OFFLINE=1` to disable all startup network operations described here, including package update checks. This does not change model catalog refresh unless you use the dedicated offline build path.
