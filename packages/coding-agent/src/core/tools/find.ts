...
function formatFindResult(
	...
	if (resultLimit || truncation?.truncated) {
		...
		if (resultLimit) warnings.push(`${resultLimit} 结果上限`);
					...
					try {
						...
						if (customOps?.glob) {
							...
							if (results.length === 0) {
								settle(() =>
									resolve({
										content: [{ type: "text", text: "未找到匹配该模式的文件" }],
										details: undefined,
									}),
								);
								return;
							...
							if (resultLimitReached) {
								notices.push(`已达 ${effectiveLimit} 结果上限`);
						...
						child.on("close", (code) => {
							...
							if (code !== 0) {
								const errorMsg = stderr.trim() || `fd 以退出码 ${code} 结束`;
								if (!output) {
							...
							if (resultLimitReached) {
								notices.push(
									`已达 ${effectiveLimit} 结果上限。使用 limit=${effectiveLimit * 2} 获取更多，或缩小模式范围`,
								);
								details.resultLimitReached = effectiveLimit;
							}
							if (truncation.truncated) {
