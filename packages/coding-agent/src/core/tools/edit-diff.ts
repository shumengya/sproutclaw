...
export function applyReplacementsPreservingUnchangedLines(
	...
	if (originalLines.length !== baseLines.length) {
		throw new Error("无法保留未更改的行，因为基础内容的行数不同。");
	}
...
function getNotFoundError(path: string, editIndex: number, totalEdits: number): Error {
	if (totalEdits === 1) {
		return new Error(
			`无法在 ${path} 中找到完全匹配的文本。old text 必须精确匹配，包括所有空白和换行。`,
		);
	}
	return new Error(
		`无法在 ${path} 中找到 edits[${editIndex}]。oldText 必须精确匹配，包括所有空白和换行。`,
	);
}
...
function getDuplicateError(path: string, editIndex: number, totalEdits: number, occurrences: number): Error {
	if (totalEdits === 1) {
		return new Error(
			`在 ${path} 中找到 ${occurrences} 处相同文本。文本必须唯一。请提供更多上下文使其唯一。`,
		);
	}
	return new Error(
		`在 ${path} 中找到 ${occurrences} 处 edits[${editIndex}]。每个 oldText 必须唯一。请提供更多上下文使其唯一。`,
	);
}

function getEmptyOldTextError(path: string, editIndex: number, totalEdits: number): Error {
	if (totalEdits === 1) {
		return new Error(`${path} 中的 oldText 不能为空。`);
	}
	return new Error(`${path} 中的 edits[${editIndex}].oldText 不能为空。`);
}

function getNoChangeError(path: string, totalEdits: number): Error {
	if (totalEdits === 1) {
		return new Error(
			`未对 ${path} 做出更改。替换产生了相同内容。这可能表示特殊字符有问题，或文本并非如预期那样存在。`,
		);
	}
	return new Error(`未对 ${path} 做出更改。替换产生了相同内容。`);
}
...
export function applyEditsToNormalizedContent(
	...
	for (let i = 1; i < matchedEdits.length; i++) {
		...
		if (previous.matchIndex + previous.matchLength > current.matchIndex) {
			throw new Error(
				`edits[${previous.editIndex}] 与 edits[${current.editIndex}] 在 ${path} 中重叠。请合并为一次编辑，或定位到不相交的区域。`,
			);
		}
	}
