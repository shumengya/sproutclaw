...
function formatGrepCall(
	...
	let text =
		...
		theme.fg("toolOutput", ` 于 ${path === null ? invalidArg : path}`);
...
function formatGrepResult(
	...
	if (matchLimit || truncation?.truncated || linesTruncated) {
		...
		if (matchLimit) warnings.push(`${matchLimit} 匹配上限`);
					...
					try {
						...
						child.on("close", async (code) => {
							...
							if (!killedDueToLimit && code !== 0 && code !== 1) {
								const errorMsg = stderr.trim() || `ripgrep 以退出码 ${code} 结束`;
								settle(() => reject(new Error(errorMsg)));
								return;
							}
							if (matchCount === 0) {
								settle(() =>
									resolve({ content: [{ type: "text", text: "未找到匹配" }], details: undefined }),
								);
								return;
							}
							...
							if (matchLimitReached) {
								notices.push(
									`已达 ${effectiveLimit} 匹配上限。使用 limit=${effectiveLimit * 2} 获取更多，或缩小模式范围`,
								);
								details.matchLimitReached = effectiveLimit;
							}
							if (truncation.truncated) {
								notices.push(`已达 ${formatSize(DEFAULT_MAX_BYTES)} 限制`);
								details.truncation = truncation;
							}
							if (linesTruncated) {
								notices.push(
									`部分行已截断至 ${GREP_MAX_LINE_LENGTH} 字符。使用 read 工具查看完整行`,
								);
