...
function validateName(name: string): string[] {
	...
	if (name.length > MAX_NAME_LENGTH) {
		errors.push(`名称超过 ${MAX_NAME_LENGTH} 个字符（${name.length}）`);
	}

	if (!/^[a-z0-9-]+$/.test(name)) {
		errors.push(`名称包含无效字符（仅允许小写 a-z、0-9 和连字符）`);
	}
...
function validateDescription(description: string | undefined): string[] {
	...
	if (!description || description.trim() === "") {
		...
		errors.push(`description 超过 ${MAX_DESCRIPTION_LENGTH} 个字符（${description.length}）`);
	}
...
function loadSkillFromFile(
	...
	try {
		...
		const message = error instanceof Error ? error.message : "解析 skill 文件失败";
...
export function loadSkills(options: LoadSkillsOptions): LoadSkillsResult {
	...
	for (const rawPath of skillPaths) {
		...
		if (!existsSync(resolvedPath)) {
			allDiagnostics.push({ type: "warning", message: "skill 路径不存在", path: resolvedPath });
			continue;
		}
		...
		try {
			...
			if (stats.isDirectory()) {
				...
				allDiagnostics.push({ type: "warning", message: "skill 路径不是 markdown 文件", path: resolvedPath });
			}
		} catch (error) {
			const message = error instanceof Error ? error.message : "读取 skill 路径失败";
