...
export class DefaultResourceLoader implements ResourceLoader {
	...
	async reload(options?: ResourceLoaderReloadOptions): Promise<void> {
		...
		for (const p of this.additionalSkillPaths) {
			if (isLocalPath(p)) {
				...
				if (!existsSync(resolved) && !this.skillDiagnostics.some((d) => d.path === resolved)) {
					this.skillDiagnostics.push({ type: "error", message: "Skill 路径不存在", path: resolved });
				}
			}
		}
		...
		for (const p of this.additionalThemePaths) {
			...
			if (!existsSync(resolved) && !this.themeDiagnostics.some((d) => d.path === resolved)) {
				this.themeDiagnostics.push({ type: "error", message: "Theme 路径不存在", path: resolved });
			}
		}
		...
		const baseAppend = appendSources
			.map((s) => resolvePromptInput(s, "追加系统提示词"))
	...
	private loadThemes(
		...
		for (const p of paths) {
			const resolved = this.resolveResourcePath(p);
			if (!existsSync(resolved)) {
				diagnostics.push({ type: "warning", message: "theme 路径不存在", path: resolved });
				continue;
			}
			...
			try {
				...
				if (stats.isDirectory()) {
					...
					diagnostics.push({ type: "warning", message: "theme 路径不是 json 文件", path: resolved });
				}
			} catch (error) {
				const message = error instanceof Error ? error.message : "读取 theme 路径失败";
	...
	private loadThemesFromDir(dir: string, themes: Theme[], diagnostics: ResourceDiagnostic[]): void {
		...
		try {
			...
			const message = error instanceof Error ? error.message : "读取 theme 目录失败";
	...
	private loadThemeFromFile(filePath: string, themes: Theme[], diagnostics: ResourceDiagnostic[]): void {
		try {
			...
			const message = error instanceof Error ? error.message : "加载 theme 失败";
