...
const ThemeJsonSchema = Type.Object({
	...
	colors: Type.Object({
		...
		mdCodeBg: ColorValueSchema,
		// Markdown (14 colors)
		mdHeading: ColorValueSchema,
		mdHeadingMuted: ColorValueSchema,
		...
		mdTableBorder: ColorValueSchema,
		mdKbd: ColorValueSchema,
		mdMark: ColorValueSchema,
		// Tool Diffs (3 colors)
	...
	if (!BUILTIN_THEMES) {
		...
		for (const file of fs.readdirSync(themesDir)) {
			...
			try {
				...
				const name = themeJson.name || file.slice(0, -".json".length);
				BUILTIN_THEMES[name] = themeJson;
			} catch (error) {
				console.error(`Failed to load built-in theme ${file}:`, error);
			}
		}
	}
...
function parseThemeJson(label: string, json: unknown): ThemeJson {
	if (!validateThemeJson.Check(json)) {
		...
		if (missingColors.size > 0) {
			...
			errorMessage += "\nSee the built-in themes in the theme/ directory for reference values.";
		}
...
export function getDefaultTheme(): string {
	...
	// SproutAI defaults dark terminals to the purple theme.
	return detected === "light" ? "light" : "purple";
}
...
export function initTheme(themeName?: string, enableWatcher: boolean = false): void {
	...
	try {
		...
		// Theme is invalid - fall back to purple theme silently
		currentThemeName = "purple";
		setGlobalTheme(loadTheme("purple"));
...
export function setTheme(name: string, enableWatcher: boolean = false): { success: boolean; error?: string } {
	...
	try {
		...
		// Theme is invalid - fall back to purple theme
		currentThemeName = "purple";
		setGlobalTheme(loadTheme("purple"));
		// Don't start watcher for fallback theme
		return {
...
export function getMarkdownTheme(): MarkdownTheme {
	return {
		...
		headingMuted: (text: string) => theme.fg("mdHeadingMuted", text),
		link: (text: string) => theme.fg("mdLink", text),
		linkUrl: (text: string) => theme.fg("mdLinkUrl", text),
		code: (text: string) => theme.bg("mdCodeBg", theme.fg("mdCode", text)),
		...
		kbd: (text: string) => theme.bg("mdCodeBg", theme.fg("mdKbd", text)),
		mark: (text: string) => theme.bg("mdCodeBg", theme.fg("mdMark", text)),
