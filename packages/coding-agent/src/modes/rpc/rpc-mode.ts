...
export async function runRpcMode(runtimeHost: AgentSessionRuntime): Promise<never> {
	...
	const createExtensionUIContext = (): ExtensionUIContext => ({
		...
		getHideThinkingBlock() {
			// Thinking visibility not supported in RPC mode - no TUI
			return false;
		},

		setHideThinkingBlock(_hidden: boolean) {
			// Thinking visibility not supported in RPC mode - no TUI
		},
	});
	...
	const handleCommand = async (command: RpcCommand): Promise<RpcResponse | undefined> => {
		...
		switch (command.type) {
			...
			case "list_agent_profiles": {
				return success(id, "list_agent_profiles", {
					profiles: listAgentProfileOptions(),
					current: { name: APP_NAME, configDir: CONFIG_DIR_NAME },
				});
			}

			case "switch_agent_profile": {
				if (session.isStreaming) {
					return error(id, "switch_agent_profile", "请等待当前回复完成后再切换配置");
				}
				if (session.isCompacting) {
					return error(id, "switch_agent_profile", "请等待压缩完成后再切换配置");
				}
				const name = command.name.trim();
				const configDir = command.configDir.trim();
				if (!name || !configDir) {
					return error(id, "switch_agent_profile", "name 和 configDir 不能为空");
				}
				setDefaultAgentProfile(name, configDir);
				setActiveAgentProfileIdentity({ name, configDir });
				const agentDir = join(getPortableAgentsRoot(), configDir, "agent");
				const result = await runtimeHost.switchAgentProfile({ agentDir });
				if (!result.cancelled) {
					await rebindSession();
				}
				return success(id, "switch_agent_profile", result);
			}
