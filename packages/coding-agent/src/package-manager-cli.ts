...
/** SproutAI fork: upstream install/update/list flows are disabled until a custom path exists. */
export const PACKAGE_COMMANDS_DISABLED = true;

export function reportPackageCommandDisabled(command: PackageCommand): void {
	console.error(chalk.yellow("该命令暂未实现"));
	console.error(chalk.dim(`${getPackageCommandUsage(command)}`));
	process.exitCode = 1;
}
...
export async function handlePackageCommand(
	...
	if (PACKAGE_COMMANDS_DISABLED) {
		reportPackageCommandDisabled(options.command);
		return true;
	}
