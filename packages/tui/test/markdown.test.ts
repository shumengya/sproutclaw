...
| Bob | 25 |`,
		...
		it("should render horizontal dividers between every row", () => {
			const markdown = new Markdown(
				`| Name | Age |
| --- | --- |
| Alice | 30 |
| Bob | 25 |`,
			...
			// Header separator + one between Alice and Bob
			assert.strictEqual(dividerLines.length, 2, "Expected a divider after header and between data rows");
			assert.ok(plainLines.some((line) => line.startsWith("┌")), "Expected top border");
			assert.ok(plainLines.some((line) => line.startsWith("└")), "Expected bottom border");
		});

		it("should keep CJK table borders vertically aligned across wrapped rows", () => {
			const markdown = new Markdown(
				`| 分类 | 具体技术 | 版本 | 依据来源 |
| --- | --- | --- | --- |
| 后端框架 | Hono | ^4.11.3 | package.json dependencies、src/server/http/app.ts |
| 前端框架 | Vue 3 + Vue Router | vue ^3.5.25、vue-router ^4.6.4 | package.json dependencies |
| 前端 UI 库 | Naive UI（另含轻量自研 i18n、主题与请求封装） | ^2.44.1 | package.json dependencies、src/client/main.ts |`,
				1,
				0,
				defaultMarkdownTheme,
			);
			...
			const width = 120;
			const plainLines = markdown.render(width).map((line) => line.replace(/\x1b\[[0-9;]*m/g, ""));
			const tableLines = plainLines.filter((line) => /[│┌├└]/.test(line));
			assert.ok(tableLines.length > 0, "Expected table lines");

			const borderColumns = (line: string): number[] => {
				...
				while (i < line.length) {
					...
					col += visibleWidth(segment);
					i += segment.length;
				}
				return positions;
			};

			const reference = borderColumns(tableLines[0]!);
			assert.strictEqual(reference.length, 5, "Expected 4-column table borders");
			for (const line of tableLines) {
				assert.deepStrictEqual(
					borderColumns(line),
					reference,
					`Misaligned table borders in: ${line.trimEnd()}`,
				);
			}

			// Wrapped multi-line body rows should still be separated by grid rules
			const midRules = plainLines.filter((line) => line.includes("┼"));
			assert.ok(midRules.length >= 3, "Expected header + per-row dividers");
		});
