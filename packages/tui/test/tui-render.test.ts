...
describe("TUI differential rendering", () => {
	...
	it("keeps the viewport on the latest turn when an above-viewport line reflows", async () => {
		const terminal = new LoggingVirtualTerminal(20, 5);
		const tui = new TUI(terminal);
		const component = new TestComponent();
		tui.addChild(component);

		// Simulate a long chat: history above the viewport + a streaming reply at the bottom.
		component.lines = Array.from({ length: 20 }, (_, i) => `Line ${i}`);
		tui.start();
		await terminal.waitForRender();
		...
		// Markdown streaming often reflows early lines of the live reply that have already
		// scrolled above the viewport. Mutate an above-viewport line and grow the buffer.
		component.lines = ["Line 0 reflowed", ...Array.from({ length: 22 }, (_, i) => `Line ${i + 1}`)];
		tui.requestRender();
		await terminal.waitForRender();

		assert.ok(tui.fullRedraws > redrawsBefore, "Above-viewport reflow should refresh the viewport");

		const writes = terminal.getWrites();
		assert.ok(!writes.includes("\x1b[2J"), "Should not clear the viewport mid-stream");
		assert.ok(!writes.includes("\x1b[3J"), "Should not clear scrollback mid-stream");
		assert.ok(!writes.includes("Line 0 reflowed"), "Should not paint the first-turn line into the refresh");

		assert.deepStrictEqual(terminal.getViewport(), ["Line 18", "Line 19", "Line 20", "Line 21", "Line 22"]);

		// Differential append after the clamped repaint must stay coherent.
		terminal.clearWrites();
		const redrawsAfterReflow = tui.fullRedraws;
		component.lines = [...component.lines, "Line 23"];
		tui.requestRender();
		await terminal.waitForRender();

		assert.strictEqual(tui.fullRedraws, redrawsAfterReflow, "Pure append should stay on the differential path");
		assert.ok(!terminal.getWrites().includes("\x1b[2J"), "Append should not clear the screen");
		assert.deepStrictEqual(terminal.getViewport(), ["Line 19", "Line 20", "Line 21", "Line 22", "Line 23"]);

		tui.stop();
	});

	it("skips painting when an above-viewport change does not shift line indices", async () => {
		const terminal = new LoggingVirtualTerminal(20, 5);
		const tui = new TUI(terminal);
		const component = new TestComponent();
		tui.addChild(component);
		...
		component.lines = Array.from({ length: 20 }, (_, i) => (i === 2 ? "Line 2 edited" : `Line ${i}`));
		tui.requestRender();
		await terminal.waitForRender();

		assert.strictEqual(tui.fullRedraws, redrawsBefore, "In-place off-screen edits should not redraw");
		assert.strictEqual(terminal.getWrites(), "", "In-place off-screen edits should not write to the terminal");
		assert.deepStrictEqual(terminal.getViewport(), ["Line 15", "Line 16", "Line 17", "Line 18", "Line 19"]);

		tui.stop();
	});

	it("keeps the editor at the bottom after an above-viewport reflow", async () => {
		...
		// Reflow early chat history and grow — mimics streaming markdown above the viewport.
		chat.lines = ["Chat 0 reflowed", ...Array.from({ length: 22 }, (_, i) => `Chat ${i + 1}`)];
		tui.requestRender();
		await terminal.waitForRender();

		const writes = terminal.getWrites();
		assert.ok(!writes.includes("\x1b[2J"), "Should not clear the viewport");
		assert.ok(!writes.includes("\x1b[3J"), "Should not clear scrollback");

		const viewport = terminal.getViewport();
		assert.deepStrictEqual(viewport, ["Chat 19", "Chat 20", "Chat 21", "Chat 22", "Editor 0", "Editor 1"]);

		// Editor must appear only as the trailing live UI — not interleaved mid-history.
		const scrollBuffer = terminal.getScrollBuffer().map((line) => line.trimEnd());
		const editorIndexes = scrollBuffer
			.map((line, index) => (line === "Editor 0" || line === "Editor 1" ? index : -1))
			.filter((index) => index >= 0);
		assert.ok(editorIndexes.length >= 2, "Editor lines should still be present");
		const firstEditor = editorIndexes[0];
		const chatAfterEditor = scrollBuffer.slice(firstEditor).some((line) => line.startsWith("Chat "));
		assert.ok(!chatAfterEditor, "Chat content must not appear after the editor in the buffer");
