...
describe("Editor component", () => {
	...
	describe("Word wrapping", () => {
		...
		it("wraps between atomic image markers instead of splitting filenames", () => {
			const a = "[屏幕截图 2026-05-13 210135.png]";
			const b = "[屏幕截图 2026-05-13 215541.png]";
			const line = `查看 ${a} ${b}`;
			const segments: Intl.SegmentData[] = [
				{ segment: "查", index: 0, input: line },
				{ segment: "看", index: 1, input: line },
				{ segment: " ", index: 2, input: line },
				{ segment: a, index: 3, input: line },
				{ segment: " ", index: 3 + a.length, input: line },
				{ segment: b, index: 4 + a.length, input: line },
			];

			const width = visibleWidth(`查看 ${a}`);
			const chunks = wordWrapLine(line, width, segments);

			assert.ok(chunks.some((chunk) => chunk.text.includes(a) && !chunk.text.includes(b)));
			assert.ok(chunks.some((chunk) => chunk.text.includes(b)));
			for (const chunk of chunks) {
				assert.ok(!chunk.text.includes("210135.png][屏幕"), "markers should not be glued/split wrongly");
			}
		});
	...
	describe("Paste marker atomic behavior", () => {
		...
		it("treats file attachment markers as single units for backspace and arrows", () => {
			const marker = "[树萌芽的作品集.zip]";
			const editor = new Editor(createTestTUI(), defaultEditorTheme, {
				getAtomicMarkers: () => [marker],
			});
			editor.setText(`看 ${marker} 完`);
			editor.handleInput("\x01"); // start of line
			const startCol = "看 ".length;
			const endCol = `看 ${marker}`.length;

			// Move to start of marker, then right should jump past whole marker
			while (editor.getCursor().col < startCol) {
				editor.handleInput("\x1b[C");
			}
			assert.deepStrictEqual(editor.getCursor(), { line: 0, col: startCol });
			editor.handleInput("\x1b[C");
			assert.deepStrictEqual(editor.getCursor(), { line: 0, col: endCol });

			// Left arrow should jump back to start of marker
			editor.handleInput("\x1b[D");
			assert.deepStrictEqual(editor.getCursor(), { line: 0, col: startCol });

			// Move to end again and backspace deletes the entire marker
			editor.handleInput("\x1b[C");
			editor.handleInput("\x7f");
			assert.strictEqual(editor.getText(), "看  完");
			assert.deepStrictEqual(editor.getCursor(), { line: 0, col: startCol });
		});

		it("deletes whole file marker even when cursor is inside it", () => {
			const marker = "[屏幕截图 2026-05-13 205547.png]";
			const editor = new Editor(createTestTUI(), defaultEditorTheme, {
				getAtomicMarkers: () => [marker],
			});
			editor.setText(marker);
			// Force cursor into the middle (simulates click landing inside)
			(editor as unknown as { state: { cursorCol: number } }).state.cursorCol = marker.length - 3;
			editor.handleInput("\x7f");
			assert.strictEqual(editor.getText(), "");
			assert.deepStrictEqual(editor.getCursor(), { line: 0, col: 0 });
		});
