...
export class TUI extends Container {
	...
	private doRender(): void {
		...
		// Above-viewport changes during streaming must not clear scrollback (`\x1b[3J`
		// jumps Windows Terminal to the top) and must not rewrite only the last screenful
		// after a viewport clear (that desyncs the physical buffer from previousLines and
		// leaves a stale editor copy mid-history). Grow the buffer if needed, then
		// rewrite the visible window in place.
		if (firstChanged < prevViewportTop) {
			const lengthDelta = newLines.length - this.previousLines.length;
			if (lastChanged < prevViewportTop && lengthDelta === 0) {
				// Off-screen in-place edit with stable indices — nothing visible changed.
				logRedraw(`skipped above-viewport in-place change (${firstChanged} < ${prevViewportTop})`);
				this.positionHardwareCursor(cursorPos, newLines.length);
				this.previousLines = newLines;
				this.previousKittyImageIds = this.collectKittyImageIds(newLines);
				this.previousWidth = width;
			...
			if (lengthDelta < 0) {
				// Shrink that invalidates layout above the viewport — need a consistent rebuild.
				logRedraw(`above-viewport shrink (${firstChanged} < ${prevViewportTop})`);
				fullRender(true);
				return;
			}

			logRedraw(`viewport-clamped repaint (${firstChanged} < ${prevViewportTop}, delta=${lengthDelta})`);
			this.fullRedrawCount += 1;
			const visibleStart = Math.max(0, newLines.length - height);
			let refreshBuffer = "\x1b[?2026h"; // Begin synchronized output
			refreshBuffer += this.deleteChangedKittyImages(firstChanged, Math.max(lastChanged, newLines.length - 1));

			if (lengthDelta > 0) {
				const currentScreenRow = Math.max(0, Math.min(height - 1, hardwareCursorRow - prevViewportTop));
				const moveToBottom = height - 1 - currentScreenRow;
				if (moveToBottom > 0) {
					refreshBuffer += `\x1b[${moveToBottom}B`;
				}
				refreshBuffer += "\r\n".repeat(lengthDelta);
				prevViewportTop += lengthDelta;
				viewportTop = prevViewportTop;
				hardwareCursorRow = Math.max(0, newLines.length - 1);
			}

			prevViewportTop = visibleStart;
			viewportTop = visibleStart;

			const screenRow = Math.max(0, Math.min(height - 1, hardwareCursorRow - prevViewportTop));
			if (screenRow > 0) {
				refreshBuffer += `\x1b[${screenRow}A`;
			}
			refreshBuffer += "\r";

			for (let i = visibleStart; i < newLines.length; i++) {
				if (i > visibleStart) refreshBuffer += "\r\n";
				const line = newLines[i];
				const isImage = isImageLine(line);
				const imageReservedRows = isImage ? this.getKittyImageReservedRows(newLines, i) : 1;
				if (imageReservedRows > 1) {
					const imageStartScreenRow = i - visibleStart;
					...
					refreshBuffer += "\x1b[2K";
					for (let row = 1; row < imageReservedRows; row++) {
						refreshBuffer += "\r\n\x1b[2K";
					}
					refreshBuffer += `\x1b[${imageReservedRows - 1}A`;
					refreshBuffer += line;
					refreshBuffer += `\x1b[${imageReservedRows - 1}B`;
					i += imageReservedRows - 1;
					continue;
				}
				refreshBuffer += "\x1b[2K";
			...
			refreshBuffer += "\x1b[?2026l"; // End synchronized output
			this.terminal.write(refreshBuffer);
			this.cursorRow = Math.max(0, newLines.length - 1);
			this.hardwareCursorRow = this.cursorRow;
			this.maxLinesRendered = Math.max(this.maxLinesRendered, newLines.length);
			this.previousViewportTop = visibleStart;
			this.positionHardwareCursor(cursorPos, newLines.length);
			this.previousLines = newLines;
			this.previousKittyImageIds = this.collectKittyImageIds(newLines);
			this.previousWidth = width;
