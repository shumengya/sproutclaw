...
/** File attachment markers like `[screenshot.png]` / `[archive.zip]` when already merged. */
const FILE_MARKER_SINGLE = /^\[[^\]\n]+\.[a-zA-Z][a-zA-Z0-9]{0,19}\]$/;

/**
 * True for multi-grapheme atomic units (paste markers, file path markers, etc.).
 * Used by word-wrap so CJK rules don't create break points inside the unit.
 */
function isAtomicWrapUnit(segment: string): boolean {
	return isPasteMarker(segment) || FILE_MARKER_SINGLE.test(segment);
}

function collectAtomicMarkerSpans(
	text: string,
	validPasteIds: Set<number>,
	atomicMarkers: readonly string[],
): Array<{ start: number; end: number }> {
	const markers: Array<{ start: number; end: number }> = [];

	if (validPasteIds.size > 0 && text.includes("[paste #")) {
	...
	if (atomicMarkers.length > 0) {
		const unique = [...new Set(atomicMarkers.filter((marker) => marker.length > 0))].sort(
			(a, b) => b.length - a.length,
		);
		const occupied = markers.map((marker) => [marker.start, marker.end] as const);
		for (const marker of unique) {
			...
			while (from < text.length) {
				...
				const overlaps = occupied.some(([start, occupiedEnd]) => index < occupiedEnd && end > start);
				if (!overlaps) {
					markers.push({ start: index, end });
 ...
 *
 * Only paste markers whose numeric ID exists in `validIds`, and exact strings
 * from `atomicMarkers`, are merged.
 */
function segmentWithMarkers(
	...
	const markers = collectAtomicMarkerSpans(text, validIds, atomicMarkers);
	if (markers.length === 0) {
...
export function wordWrapLine(line: string, maxWidth: number, preSegmented?: Intl.SegmentData[]): TextChunk[] {
	...
	for (let i = 0; i < segments.length; i++) {
		...
		const isWs = !isAtomicWrapUnit(grapheme) && isWhitespaceChar(grapheme);
		...
		if (isWs && next && (isAtomicWrapUnit(next.segment) || !isWhitespaceChar(next.segment))) {
			wrapOppIndex = next.index;
			wrapOppWidth = currentWidth;
		} else if (!isWs && next && !isWhitespaceChar(next.segment)) {
			const isCjk = !isAtomicWrapUnit(grapheme) && cjkBreakRegex.test(grapheme);
			const nextIsCjk = !isAtomicWrapUnit(next.segment) && cjkBreakRegex.test(next.segment);
			if (isCjk || nextIsCjk) {
				wrapOppIndex = next.index;
				wrapOppWidth = currentWidth;
			}
...
export interface EditorOptions {
	...
	/** Optional display-only styling (must not change visible character count). */
	styleDisplayText?: (text: string) => string;
	/**
	 * Exact marker strings that should be treated as atomic units for cursor
	 * movement and word-wrap (e.g. `[screenshot.png]` image attachments).
	 */
	getAtomicMarkers?: () => Iterable<string>;
}
...
export class Editor implements Component, Focusable {
	...
	private styleDisplayText?: (text: string) => string;
	private getAtomicMarkers?: () => Iterable<string>;

	public onSubmit?: (text: string) => void;
	public onChange?: (text: string) => void;
	/**
	 * Optional paste interceptor. Return true to skip default insertion
	 * (e.g. convert image file paths into attachment markers).
	 */
	public onPasteContent?: (text: string) => boolean;
	...
	constructor(tui: TUI, theme: EditorTheme, options: EditorOptions = {}) {
		...
		this.styleDisplayText = options.styleDisplayText;
		this.getAtomicMarkers = options.getAtomicMarkers;
	}

	private applyStyleDisplayText(text: string): string {
		return this.styleDisplayText ? this.styleDisplayText(text) : text;
	}
	...
	private atomicMarkerList(): string[] {
		if (!this.getAtomicMarkers) {
			return [];
		}
		return [...this.getAtomicMarkers()];
	}

	/** Segment text with paste/image-marker awareness. */
	private segment(text: string, mode: "word" | "grapheme"): Iterable<Intl.SegmentData> {
		return segmentWithMarkers(
			...
			this.validPasteIds(),
			this.atomicMarkerList(),
		);
	}

	/** Atomic marker spans on a logical line (paste markers + getAtomicMarkers). */
	private atomicSpansOnLine(line: string): Array<{ start: number; end: number }> {
		return collectAtomicMarkerSpans(line, this.validPasteIds(), this.atomicMarkerList());
	}

	/**
	 * Find an atomic marker that should be deleted/skipped as a whole.
	 * - backspace: cursor at end of or inside marker
	 * - forward: cursor at start of or inside marker
	 */
	private findAtomicSpanForEdit(
		line: string,
		col: number,
		mode: "backspace" | "forward",
	): { start: number; end: number } | undefined {
		const spans = this.atomicSpansOnLine(line);
		if (mode === "backspace") {
			return spans.find((span) => col > span.start && col <= span.end);
		}
		return spans.find((span) => col >= span.start && col < span.end);
	}
		...
		for (const layoutLine of visibleLines) {
			...
			if (layoutLine.hasCursor && layoutLine.cursorPos !== undefined) {
				...
				if (after.length > 0) {
					...
					displayText =
						this.applyStyleDisplayText(before) + marker + cursor + this.applyStyleDisplayText(restAfter);
					// lineVisibleWidth stays the same - we're replacing, not adding
				} else {
					// Cursor is at the end - add highlighted space
					const cursor = "\x1b[7m \x1b[0m";
					displayText = this.applyStyleDisplayText(before) + marker + cursor;
				...
				displayText = this.applyStyleDisplayText(rawText);
			}
	...
	private handlePaste(pastedText: string): void {
		...
		// Allow callers (coding-agent) to intercept image-path pastes before insertion.
		if (this.onPasteContent?.(filteredText)) {
			if (this.onChange) {
				this.onChange(this.getText());
			}
			return;
	...
	private handleBackspace(): void {
		...
		if (this.state.cursorCol > 0) {
			...
			const line = this.state.lines[this.state.cursorLine] || "";
			const atomicSpan = this.findAtomicSpanForEdit(line, this.state.cursorCol, "backspace");
			if (atomicSpan) {
				this.state.lines[this.state.cursorLine] = line.slice(0, atomicSpan.start) + line.slice(atomicSpan.end);
				this.setCursorCol(atomicSpan.start);
			} else {
	...
	private handleForwardDelete(): void {
		...
		if (this.state.cursorCol < currentLine.length) {
			...
			const atomicSpan = this.findAtomicSpanForEdit(currentLine, this.state.cursorCol, "forward");
			if (atomicSpan) {
				this.state.lines[this.state.cursorLine] =
					currentLine.slice(0, atomicSpan.start) + currentLine.slice(atomicSpan.end);
				this.setCursorCol(atomicSpan.start);
			} else {
		...
		if (deltaCol !== 0) {
			...
			if (deltaCol > 0) {
				...
				if (this.state.cursorCol < currentLine.length) {
					const atomicSpan = this.findAtomicSpanForEdit(currentLine, this.state.cursorCol, "forward");
					if (atomicSpan) {
						this.setCursorCol(atomicSpan.end);
					} else {
						const afterCursor = currentLine.slice(this.state.cursorCol);
						const graphemes = [...this.segment(afterCursor, "grapheme")];
						const firstGrapheme = graphemes[0];
				...
				if (this.state.cursorCol > 0) {
					const atomicSpan = this.findAtomicSpanForEdit(currentLine, this.state.cursorCol, "backspace");
					if (atomicSpan) {
						this.setCursorCol(atomicSpan.start);
					} else {
						const beforeCursor = currentLine.slice(0, this.state.cursorCol);
