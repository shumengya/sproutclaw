...
interface MarkdownTheme {
  ...
  headingMuted?: (text: string) => string; // H3–H6; falls back to heading
  ...
  tableBorder?: (text: string) => string; // falls back to codeBlockBorder
  kbd?: (text: string) => string;          // HTML <kbd>; falls back to code
  mark?: (text: string) => string;         // HTML <mark>; falls back to bold+underline
...
**Features:**
- Headings (H1–H6, no `#` prefix), bold, italic, code blocks, lists, links, blockquotes
- Clean tables with themed borders; unordered lists use `•` / `◦`
- Common inline/block HTML rendered semantically (`b`/`i`/`kbd`/`mark`/`details`/…)
...
2. **Width/Height Changed**: Clear screen and full re-render
3. **Change Above Viewport**: Viewport-clamped repaint — grow the buffer if needed, then rewrite the visible window in place (no scrollback clear), so long streaming stays pinned to the current turn without desyncing history
4. **Normal Update**: Move cursor to first changed line, clear to end, render changed lines
